...
2. **Repo hardening:** README, `requirements.txt`, pinned deps, 6-vehicle launcher, install instructions verified from a clean shell. *(Audited 25 Sep — see A19: run command confirmed working from a clean shell; `requirements.txt` missing; README has no clone step; launcher script content is current but this line's “6-vehicle” wording is stale — not yet corrected.)*
...
- **A19 — Week 3 item 2 (repo hardening) audited from a clean shell; two of four sub-items incomplete, and the “6-vehicle launcher” wording is stale.** Fresh-venv audit (25 Sep, `verify_week3_fresh.sh`): (i) root `requirements.txt` — **missing** (the stdlib-only design may make this moot, but the sprint item asks for a pinned file and none exists); (ii) README install instructions — run/score/view commands verified to work verbatim, but **no `git clone` step is documented anywhere**, so a stranger without the repo already in hand cannot follow it end-to-end; (iii) documented run command (`python3 -m uavx.survey`) — **confirmed working** from a genuinely fresh venv, real telemetry output produced, exit code 0; (iv) `launch_sitl_4uav.sh` — **exists and is current** (`--count 4`, matches the 4-airframe fleet already used in the README's own examples), but the checklist line above still reads “6-vehicle launcher,” which nobody has corrected.

  Separately, and more seriously: this audit surfaced that the fleet-finalized-to-4 / Eye-relay-capable / 120°-spacing rulings — previously treated as settled and cited elsewhere as A20/A21 — **do not appear anywhere in this document.** The live Amendment Log stops at A18. Either those rulings were drafted and never committed here, or they were never actually ratified. Until Master resolves which, treat the 4-vehicle fleet as **implemented in code but not formally ratified**, and do not cite A20/A21 as authoritative — they are not in this constitution.

*End of operations document. Last amended 25 September 2026 (A19, Week 3 repo-hardening audit + fleet-ruling gap flagged).*
